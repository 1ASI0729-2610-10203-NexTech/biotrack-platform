...
public class ConsumptionRecordRepository(AppDbContext context)
    : BaseRepository<ConsumptionRecord>(context), IConsumptionRecordRepository
{
    public async Task<IEnumerable<ConsumptionRecord>> FindByPatientUserIdAndDateRangeAsync(
        int patientUserId, DateOnly start, DateOnly end, CancellationToken cancellationToken)
    {
        return await Context.Set<ConsumptionRecord>()
            .Where(r => r.PatientUserId == patientUserId && r.Date >= start && r.Date <= end)
            .ToListAsync(cancellationToken);
    }

    public async Task<IEnumerable<ConsumptionRecord>> FindByPatientUserIdAndWeekAsync(
